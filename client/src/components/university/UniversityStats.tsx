...
export default function UniversityStats({ university }: UniversityStatsProps) {
  const stats = [
    { icon: Award, label: 'World Ranking', value: university.worldRanking ? `#${university.worldRanking}` : 'Unranked' },
    { icon: TrendingUp, label: 'Acceptance Rate', value: formatPercent(university.acceptanceRate) },
    { icon: DollarSign, label: 'Tuition Range', value: `${formatMoney(university.tuitionMin)} – ${formatMoney(university.tuitionMax)}` },
    { icon: GraduationCap, label: 'Min. CGPA', value: university.minCgpa.toFixed(2) },
    { icon: Languages, label: 'Min. IELTS', value: university.minIelts ? university.minIelts.toFixed(1) : 'Not required' },
    { icon: CalendarClock, label: 'Application Deadline', value: formatDate(university.applicationDeadline) },
  ]
